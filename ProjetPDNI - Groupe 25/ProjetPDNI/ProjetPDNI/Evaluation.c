...
void testSetEvaluation(void) {
	...
	if (fiTestSet != NULL && fiModel != NULL) {
		...
		int resultsPerClass[NB_MOVEMENTS][NB_MOVEMENTS] = { 0 };
		int theoricalAppearencePerClass[NB_MOVEMENTS] = { 0 };
		int correctResultsPerClass[NB_MOVEMENTS] = { 0 };

		double percentageCorrectResultPerClass[NB_MOVEMENTS] = { 0 };
...
void convertFileToTable(FILE* file, Models models[]) {
	...
	while (!feof(file) && lineModel <= NB_MOVEMENTS) {
		...
		double acc = getValueFile(file);
		while (iVacc <= 601 && acc != -1) {
			models[lineModel - 1].v_acc[iVacc - 1] = acc;
			acc = getValueFile(file);
			iVacc++;
		}
		lineModel++;
	}
